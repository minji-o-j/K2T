...
def sample_sentence(
    ...
    pred_word, predicted_text, predicted_index, this_sequence = get_prediction(
        tokenizer, indexed_tokens, indexed_this_seq, keywords_gpt, predicted_index, guarantee, T_time, time
    )
        ...
        for b in range(number_of_beams):
            ...
            for i in range(number_of_concurrent_sentences):
                ...
                if guide[b]:
                    ...
                    for j in range(number_of_words_per_sentence):
                        context, proba, this_sequence, eos_c = sample_sentence_noguide(
                            context, this_sequence, tokenizer, model, top_p=top_p, temperature=temperature, prev_proba=proba, eos_c=eos_c, det_BS=det_BS, ith=i
                        )
...
def get_savefile(args):
    ...
    save_file = (
        ...
        + str(args.n_beams)
        + "_nGenSent_"
        + str(args.n_generated_sentences)
        + "_nWordsPerSent_"
        + str(args.n_words_per_sentence)
        + "_topP_"
        + str(args.top_p)
    )
...
def get_args(parser):
    ...
    parser.add_argument(
        "-mode", type=str, default="max", choices=["max", "next", "all", "random", "best_tour"], help="modes: max, next, all, random, best_tour"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="50keywords",
        choices=["50keywords", "ROC", "key2article", "commongen"],
        help="tasks: 50keywords, ROC, key2article, commongen",
    )
...
if __name__ == "__main__":
    ...
    if not file_name:
        raise Exception(
            "file_name name missing. Please give the relative path to word_sets filename (or the word_sets folder in case of key2article flag is True)."
        )
